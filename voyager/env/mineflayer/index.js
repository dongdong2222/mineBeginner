...
const CurrentStatus = require("./lib/observation/current_status")
const OnDectection = require("./lib/observation/onDetection")
...
app.post("/start", (req, res) => {
    ...
    bot = mineflayer.createBot({
        host: "127.0.0.1", //"localhost", // minecraft server ip
    ...
    bot.once("spawn", async () => {
        ...
        //kill 전에 tp해서 observation에 반영되게
        ...
        // spawn 완료 전 tp해서 observation에 반영되게
        if (req.body.position) {
            bot.chat(
                `/tp @s ${req.body.position.x} ${req.body.position.y} ${req.body.position.z}`
            );
...
app.post("/step", async (req, res) => {
    ...
    function onTick() {
        ...
        if (bot.globalTickCounter % 20 === 0){
            bot.chat(`dfdf ${req.body.criteria['health']}`);
            // bot.chat(`dfdf ${req.body.criteria['health'][0]}`);
            // bot.chat(`dfdf ${req.body.criteria['health'][1]}`);
            monitor_factor();
        }
    ...
    function monitor_factor()
    {
        if (Object.keys(req.body.criteria).length === 0)
            return
        let criterias = req.body.criteria
        //factors = {factor_name: ['less', value]}
        let factors = {
            health: bot.health,
            food: bot.food,
        ...
        bot.chat(`criteria_keys ${criteria_keys.length}`);

        for (let i=0; i < criteria_keys.length; i++){
            let criteria = criterias[criteria_keys[i]]
            let factor = factors[criteria_keys[i]]
            let check = criteria[0] === 'less'?"less":"greater"
            bot.chat(`iter ${i}: ${criteria}, ${factor}, ${check}`);

            if (check === "less") {
                if (factor < criteria[1]) {
                    bot.emit("detect", criteria_keys[i], factor);
                    res.json(bot.observe());
                    bot.removeListener("physicTick", onTick);
                }
            }
    ...
    function setSpectator(){
        ...
        bot.chat('/gamemode spectator dongg')
        bot.chat('/spectate @s dongg')

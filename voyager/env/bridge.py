...
class VoyagerEnv(gym.Env):
    ...
    def monitor(self):
        res = requests.post(f"{self.server}/monitor")
        if res.status_code != 200:
            raise RuntimeError("Failed to monitor Minecraft server")
        returned_date = res.json()
        return returned_date

    def set_evaluation(self):
        res = requests.post(f"{self.server}/set_evaluation")
        if res.status_code != 200:
            raise RuntimeError("Failed to set evaluation Minecraft server")
        returned_date = res.json()
        return returned_date

async function craftItem(bot, name, count = 1) {
    ...
    if (typeof name !== "string") {
        throw new Error("name for moveBack must be a string");
    }
    // return if count is not number
    if (typeof count !== "number") {
        throw new Error("count for moveBack must be a number");
    }
    ...
    if (recipe) {
        ...
        if (_craftItemFailCount > 10) {
            throw new Error(
                "moveBack failed too many times, check chat log to see what happened"
            );
        }
    }
}

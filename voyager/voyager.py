...
class Voyager:
    ...
    def learn(self, reset_env=True):
        if self.resume:
            ...
            # server pause 상태
            self.resume = True
        self.last_events = self.env.step("", last_position={"x": 246.551, "y": 82.001, "z": 164.175}) # reset 후 현재 observation만 얻기 위해 server unpaused
        # server paused 상태
        while True:
            ...
            try:
                messages, reward, done, info = self.rollout( # server unpaused
                    task=task,
                    context=context,
                    reset_env=reset_env,
                )
...
#########################################
    ...
    def test_rollout(self, *, task, context, reset_env=True):
        self.reset(task=task, context=context, reset_env=reset_env) #retrive skill 여기서 함...
        while True:
            messages, reward, done, info = self.test_step()
            if done:
                break
        return messages, reward, done, info
    ...
    def test_step(self):
        ...
        if isinstance(parsed_result, dict):
            ...
            events = self.env.step(   #게임에서 스탭
                code,
                programs=self.skill_manager.programs,
                # last_position=self.last_events[-1][1]["status"]["position"],
            )
            self.recorder.record(events, self.task)
            self.action_agent.update_chest_memory(events[-1][1]["nearbyChests"])
            success, critique = self.critic_agent.check_task_success(
    ...
    def test(self, reset_env=True):
        self.env.reset(
            options={
                "mode": "hard",
                # 고정 위치로 시작하기 (현재 : test장)
                # "position": {"x": 246.551, "y": 82.001, "z": 164.175}
            }
        )
        self.last_events = self.env.step("")
        # self.last_events = self.env.step("", last_position={"x": 246.551, "y": 82.001, "z": 164.175})
        while True:
            if self.recorder.iteration > self.max_iterations:
                print("Iteration limit reached")
                break
            ...
            try:
                message, reward, done, info = self.test_rollout(task=task, context=context, reset_env=reset_env)
            except Exception as e:
                time.sleep(3)  # wait for mineflayer to exit
                info = {
                    "task": task,
